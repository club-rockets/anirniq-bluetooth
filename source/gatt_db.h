...
PRIMARY_SERVICE_UUID128(service_charges, uuid_charges)
	...
	CHARACTERISTIC_UUID128(charges_state, uuid_charges_state, (gGattCharPropRead_c))
		VALUE_UUID128_VARLEN(charges_state_value, uuid_charges_state, (gPermissionFlagReadable_c), gAttMaxWriteDataSize_d(gAttMaxMtu_c), 1, 0x00)

	CHARACTERISTIC_UUID128(charges_enable, uuid_charges_Enable, (gGattCharPropWriteWithoutRsp_c))
		VALUE_UUID128_VARLEN(charges_enable_value, uuid_charges_Enable, (gPermissionFlagWritable_c), gAttMaxReadDataSize_d(gAttMaxMtu_c), 1, 0x00)
